...
export class OzonAdapter implements MarketplaceAdapter {
  ...
  async fetchNewOrders(_since?: Date): Promise<RawOrder[]> {
    // ВАЖНО: cutoff_from/cutoff_to фильтруют СРОК ОТГРУЗКИ постинга, а НЕ время
    // создания/обновления заказа. Поэтому since (время заказа/импорта) сюда НЕ
    // подмешиваем: свежий since сдвинул бы нижнюю границу вперёд и спрятал бы
    // ПРОСРОЧЕННЫЕ необработанные постинги (срок отгрузки в прошлом). Берём широкое
    // окно now-30д … now+60д — попадают и просроченные, и дальние. Повтор уже
    // импортированных заказов гасит идемпотентность (imported/*), а не это окно.
    const dayMs = 24 * 3600 * 1000;
    const cutoffFrom = new Date(Date.now() - 30 * dayMs).toISOString();
