...
- **НОВОЕ — импорт финансов площадок** (`finance.ts`): выгрузка финотчёта WB
  (Statistics API `reportDetailByPeriod`, курсор `rrdid`) и Ozon
  (`/v3/finance/transaction/list`, постранично) + **парсеры файлов кабинета**
  (XLSX/CSV, мапинг колонок по русским заголовкам). Чистые парсеры → строки
  реестра (категория Продажа/Возврат/Штраф/Комиссия/Логистика/Хранение + знак),
  идемпотентность по `doc_ref`. Тестируется без сети.
...
npm test            # vitest run — 55 тестов (адаптеры + finance.ts, без сети)
npm run build       # tsc → dist/
```

Ревью: правки сверять с источником в основном проекте SmartStock
(`core/integration/MarketplaceClient`); построители/парсеры (в т.ч. `finance.ts`)
держать чистыми — тестируемыми без сети и БД.
